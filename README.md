...
В режиме `CommonMark` текст длиннее 4096 символов после разбора автоматически
разбивается на сообщения, по возможности между строками или словами. Оформление,
код и ссылки сохраняются в каждой части; emoji не разрезаются пополам. Между
частями выдерживается пауза. Для длинных сообщений оформление передаётся через
Telegram entities, поэтому длина URL и служебные символы разметки не занимают
лимит видимого текста. Остальные режимы отправляются без преобразования.

Ссылки на задачи формирует `read_changelog_action`; Telegram-экшон сохраняет
готовые Markdown-ссылки при форматировании и разбиении.

Выходы `message_ids` (JSON-массив) и `message_count` содержат успешно
отправленные текстовые сообщения. При ошибке посреди отправки экшон завершается
с ошибкой и сохраняет эти выходы; повторный запуск автоматически не исключает
уже отправленные части. Не перезапускайте такую публикацию целиком без проверки,
чтобы не получить дубликаты.
